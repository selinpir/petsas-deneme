...
namespace petsas2.Services
{
    public class AdminService : IAdminService
    {
        ...
        public async Task<bool> AdminKullaniciSilAsync(int hesapId)
        {
            var hesap = await _context.HBilgileri.FindAsync(hesapId);

            if (hesap == null)
                return false;

            hesap.HesapDurumu = HesapDurumu.AdminSildi;
            _context.HBilgileri.Update(hesap);
